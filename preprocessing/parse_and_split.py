...
train_file = Path(__file__).parent / f"train.csv"
valid_file = Path(__file__).parent / f"valid.csv"
test_file = Path(__file__).parent / f"test.csv"
...
for file_path in corpus_dir.glob("**/*.xml"):
    ...
    try:
        ...
        sent_list = [tree.find("headline").text] # The headline is always the first sentence of the list.
...
print("\nAll documents parsed. Processing information...")
...
# List codes with frequency > 0, calculate weight for each code (negative / positive) and save them in a .csv file

mask = code_df["count"] != 0
code_df = code_df[mask].reset_index(drop=True)
code_df["weights"] = code_df["count"].apply(lambda x: (n_samples-x)/x)
code_df.to_csv("codecounts.csv")
...
print("Forming validation and test sets. This will take a short while.")
...
print(f"Saved results to files: {train_file}, {valid_file}, {test_file}")
print("Failed files:")
print(failed)
